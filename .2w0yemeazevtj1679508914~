 ...
 const path = require('path')
  ...
  const tm = process.uptime(),Tm=(require('moment-timezone')).tz('Asia/Ho_Chi_Minh').format('HH:mm:ss | DD/MM/YYYY')
   ...
   const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex((threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX )})\\s*`);
       ...
       if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {return api.sendMessage('⚠️ Bot đang bảo trì', threadID, messageID) 
       }
     ...
     //END --------------<
       if (!command) {
           ...
           if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
				 return api.sendMessage(`❎ Lệnh bạn sử dụng không tồn tại gõ ${threadSetting.PREFIX||PREFIX}menu để xem các lệnh hiện có\n✏️ Lệnh gần giống là: ${checker.bestMatch.target}\n────────────────\n⏱️ Upt ${H+$+M+$+S}\n⏰ Time: ${Tm}`, threadID, messageID)
       }
     if(command) {
